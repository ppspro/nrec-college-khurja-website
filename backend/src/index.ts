...
const formattedOrigins = allowedOrigins.map(origin => origin.replace(/\/$/, ''));
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      if (formattedOrigins.includes(normalizedOrigin) || normalizedOrigin.startsWith('http://localhost:')) {
        callback(null, true);
      } else {
